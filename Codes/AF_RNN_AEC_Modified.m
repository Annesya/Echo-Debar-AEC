...
b0 = 0.96; % Parameter Beta Vary from 15 to 30 in steps of 5
...
for i = 2:tidx
   ...
   % A(:,i) = 1./(1+exp(-b*(g(:,i)-1.5)));
    b = b0*(min(1,((W(:,i-1).*x_abs(:,i-1)).^2)./(err(:,i).^2)));
%     b = b0*(min(1,(err(:,i).^2))./((W(:,i-1).*x_abs(:,i-1)).^2));
    Alpha(:,i) = err(:,i)./Xpast;
...
% where Gain(f,k) = 0.5+0.5*sqrt((p-1)/p) where p is the a-posteriori SNR
   ...
   y_phase = angle(x_stft);
   s_estm = istft(s_estimated_af.*exp(sqrt(-1)*phase_all),f_new,'Window',hamming(winLen,'periodic'),'OverlapLength',overlap,'FFTLength',fftLen);
   y_estm = istft(y_estimated_af.*exp(sqrt(-1)*y_phase),f_new,'Window',hamming(winLen,'periodic'),'OverlapLength',overlap,'FFTLength',fftLen);
   s_estm = double(real(s_estm));
   y_stem = double(real(y_estm));
   %%
   s_est_abs = abs(s_estimated_af);
   input1 = log10(s_est_abs.^2);
   numFeatures = size(input1,1); tind = size(input1,2); numSegments = 1;
...
save('AF_RNN_3','aecNetFullyConnected');
%%
